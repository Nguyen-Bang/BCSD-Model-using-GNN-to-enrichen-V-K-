...
def compute_similarity_accuracy(
    embeddings_a: torch.Tensor, embeddings_b: torch.Tensor, threshold: float = 0.5
) -> dict[str, float]:
    """
...
def log_embedding_statistics(embeddings: torch.Tensor, prefix: str = "train") -> dict[str, float]:
    """
...
def compute_validation_metrics(
    model, val_loader, loss_fn, device: str = "cuda"
) -> dict[str, float]:
    """
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            # Compute loss
            loss = loss_fn(embeddings_a=embeddings_a, embeddings_b=embeddings_b)
...
def test_metrics():
    ...
    embeddings_b = (
        embeddings_a + torch.randn(batch_size, embedding_dim) * 0.1
