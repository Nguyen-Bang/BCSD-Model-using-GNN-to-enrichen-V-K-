...
- Tested "je" instruction tokenization to verify handling of missing tokens.
- Performed comprehensive vocabulary coverage check against common x64 tokens.
- Scanned project for `AssemblyTokenizer` references and fixed leftovers.
- Updated `inference/vectorizer.py` to use `ClapASMTokenizer`.
- Updated `dataset/code_dataset.py` and `dataset/collate.py` to support pre-tokenized inputs (`token_ids`) and new edge format.
- Rewrote `tests/test_preprocessing.py` to verify `ClapASMTokenizer` and batch processing.
...
- **Missing Instructions**: `je` instruction is missing from CLAP-ASM vocabulary but is handled via subword tokenization (`j` + `e`). No manual normalization required.
- **Vocabulary Coverage**: CLAP-ASM covers ~91% of common x64 tokens checked. Notable missing items (`esp`, `rip`, `ymm0`) are handled via subwords.
